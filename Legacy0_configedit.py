...
def ConfigExport2Json(configdata,configfilename = False,owconf = True):
    ...
    if cm0 == "Y" or cm0 =="y":
        fw = open(configfiledirectory + "/" + configname,'w')
...
def readconfigfile(configfile):
    if configfile and os.path.isfile(configfiledirectory + "/" + configfile):
        cfr = open(configfiledirectory + "/" + configfile,'r')
...
def configdataedit(configfile = ""):
    ...
    print("If you don't input anything,'configname'value(" + editconfigdata["configname"] + ") will be used.")
    configfilename = input(" > ")
    if configfilename == "":
        configfilename = editconfigdata["configname"]
    ConfigExport2Json(editconfigdata,configfilename)
    """
